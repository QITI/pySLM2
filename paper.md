...
In addition to these tools, `pySLM2` offers a generic API that is easily scalable across different SLM and DMD models, seamlessly integrating hardware controls with our Fourier hologram generation tools. Beyond implementing widely-used techniques, as mentioned previously, we have also incorporated a novel Iterative Fourier Transform Algorithm (IFTA) developed by the authors and colleagues. We hope that the addition of pySLM2 to the community will facilitate researchers in applying Fourier holographic beam shaping ideas on various machines with fewer hardware hurdles.
